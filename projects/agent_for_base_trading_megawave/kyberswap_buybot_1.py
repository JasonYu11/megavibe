...
def de_key_from_word(word):
    cmd = 'reg query "HKEY_CURRENT_USER\\Environment" /v nick'
    result = subprocess.check_output(cmd, shell=True).decode('utf-8')
    python_file = result.split('REG_SZ')[1].strip()
    cmd_salt = 'reg query "HKEY_CURRENT_USER\\Environment" /v matt'
...
def kyberswap_swap(token_in, token_out, amount_in,word, slippage_tolerance=800,chain='base',rate=1):
    ...
    #改为自己的钱包地址
    my_wallet_address="0x05d286a1edd8e985bbed32acf11f0135c1d40ced"    
            ...
            if do_swap:
                ...
                try:
                    ...
                    if tx_receipt['status'] == 1:
                        ...
                        print(f"交换交易失败，区块号: {tx_receipt['blockNumber']}")
                        return None, None, None, None, None, None, None
...
if condition == 1:
    ...
    eth_spent, tokenout_amount_real, amount_in_real, time_used, price_SP, tx_hash, balance_virtual = kyberswap_swap(
        ...
        "0xdddF73c5535EEC1D6Aa7f41880cbE69A64066eb1", 
        0.00001, 
        'aaaaaaaaaaa',
        800
    )
