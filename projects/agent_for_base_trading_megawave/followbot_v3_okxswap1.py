...
from okxswap_buybot_v1 import okx_swap_mev
...
class virtual_follow_bot_V2:
    def __init__(self, 
        ...
        # 外盘跟单参数
        self.waipan_buy_rate = 0.45*10**-12
        self.waipan_sell_rate = 0.45
        self.neipan_buy_rate = 1
        self.neipan_sell_rate = 1
        self.waipan_buy_max = 1700
       ...
       # no_follow_contract_list=['0x55fF51DA774b8ce0ed1ABAeD1CB76236bc6b2f16','0x2D90785E30A9df6ccE329c0171CB8Ba0f4a5c17b']
        ...
        for hash_key, txs in filtered_hashes.items():
           ...
           # )
            ...
            #input("按回车键继续...")
            if merged_txs:                
                if merged_txs[0]["tokenName"]==merged_txs[1]["tokenName"]:
                    continue
  ...
  # &apikey=YourApiKeyToken
    ...
    def send_transaction_notification_basescan(self: dict,data: dict) -> bool:
        ...
        for tx in data:
            ...
            #input("按回车键继续...")
            # 筛选逻辑
            USDC_address = cs("0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913")
            if tx_hash not in self.buyed_txs:
                ...
                if time_now - int(receive_timestamp) < 60 * self.time_follow_limit:
                    ...
                    if trade_type == "buy":
                        ...
                        if tx_hash is None and total_time is  None:
                            tx_hash, total_time = okx_swap_mev(
                                USDC_address,
                                cs(receive_contract),
                                send_value_real*waipan_buy_rate, 
                        ...
                        if tx_hash is not None:
                            message_buy = (
                                f"🚨 <b>[{self.name}] 跟单(BUY): 用时 {total_time:.1f} 秒买入 {send_value_real:.1f} USDC 的外盘代币 {receive_token_symbol}</b>\n"
                                f"💰 跟单系数: {waipan_buy_rate:.2f}\n"
                            ...
                            message_buy_error = (
                                ...
                                f"🚨 <b>[{self.name}] 买入 {send_value_real:.1f} USDC的外盘代币 {receive_token_symbol}</b>\n\n"
                        ...
                        if tx_hash is not None:
                            message_sell = (
                                f"🚨 <b>[{self.name}] 跟单(SELL): 用时 {total_time:.1f} 秒卖出 {send_value:.1f} {send_token_symbol} 的外盘代币 {receive_token_symbol}</b>\n"
                                f"💰 跟单系数: {self.waipan_sell_rate:.2f}\n"
                            ...
                            message_sell_error = (
                                f"🚨 <b>[{self.name}] 跟单(SELL)卖出失败: {send_token_symbol} 余额为 0</b>\n\n"
                                f"🚨 <b>[{self.name}] 卖出 {send_value:.1f} {send_token_symbol} 的外盘代币 {receive_token_symbol}</b>\n\n"
                                f"🔍 代币详情: <a href='https://dexscreener.com/base/{send_contract}'>查看dexscreener</a>\n"
                                f"🔗 跟单地址: <a href='https://debank.com/profile/{self.my_address}'>查看debank详情</a>\n"
                                f"🔍 查看debank: <a href='https://debank.com/profile/{self.target_address}'>查看debank</a>\n"
                                f" Mega_trade_bot V3 Powered by MEGAWAVE investment Lab 2025/08\n"
                            ...
                            if tx_hash is not None:
                                message_buy = (
                                    f"🚨 <b>[{self.name}] 跟单(BUY/EXCHANGE): 用时 {total_time:.1f} 秒买入 {send_value_real:.1f} USDC 的外盘代币 {receive_token_symbol}</b>\n"
                                    f"💰 跟单系数: {waipan_buy_rate:.2f}\n"
                                    f"🔍 代币详情: <a href='https://dexscreener.com/base/{send_contract}'>查看dexscreener</a>\n"
                                    f"🔗 跟单地址: <a href='https://debank.com/profile/{self.my_address}'>查看debank详情</a>\n"
                            ...
                            if tx_hash is not None:
                                message_sell = (
                                    f"🚨 <b>[{self.name}] 跟单(SELL/EXCHANGE):用时 {total_time:.1f} 秒卖出 {send_value:.1f} 的外盘代币 {send_token_symbol}</b>\n"
                                    f"💰 跟单系数: {self.waipan_sell_rate:.2f}\n"
                                    f"🔍 代币详情: <a href='https://dexscreener.com/base/{send_contract}'>查看dexscreener</a>\n"
                                    f"🔗 跟单地址: <a href='https://debank.com/profile/{self.my_address}'>查看debank详情</a>\n"
                                    f"{'-' * 60}\n"  # 下方分界线
